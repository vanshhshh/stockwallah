...
export function CourseCard({ course }: { course: Course }) {
  ...
  return (
    <article className="card group flex min-h-[420px] flex-col overflow-hidden transition hover:border-gold-primary/45 hover:shadow-gold">
      <div className="relative h-56 overflow-hidden bg-black-elevated">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h3 className="text-xl font-semibold leading-7 text-white-primary">{course.title}</h3>
        ...
        <div className="mt-auto flex items-end justify-between gap-4 pt-6">
          ...
          <Link href={`/enroll?course=${course.slug}`} className="gold-gradient-bg rounded px-4 py-3 text-sm font-semibold text-black-primary transition group-hover:shadow-gold">
            Enroll Now
          </Link>
        </div>
      </div>
